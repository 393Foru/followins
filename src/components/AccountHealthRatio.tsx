  ...
  return (
    <div className="w-full h-full flex flex-col bg-white border border-zinc-200 rounded-3xl p-6 md:p-8 shadow-sm relative overflow-hidden">
      <div className="flex items-center gap-3 mb-6 relative z-10">
        <div className={`p-2.5 rounded-lg ${isHealthy ? 'bg-emerald-50 text-emerald-500' : 'bg-rose-50 text-rose-500'}`}>
          <Activity className="w-6 h-6" />
        </div>
        <div>
          <h3 className="font-bold text-lg text-zinc-900 tracking-tight">{t('healthTitle')}</h3>
          <p className="text-sm text-zinc-500">{t('healthDesc')}</p>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center relative z-10 py-4">
        ...
        <div className="relative w-48 h-24 overflow-hidden mb-2">
          <div className="absolute top-0 left-0 w-48 h-48 rounded-full border-[16px] border-zinc-100 box-border"></div>
          <motion.div 
            initial={{ rotate: -180 }}
            whileInView={{ rotate: -180 + (percentage * 1.8) }}
            transition={{ duration: 1.5, ease: "easeOut" }}
            viewport={{ once: true }}
            className={`absolute top-0 left-0 w-48 h-48 rounded-full border-[16px] border-b-transparent border-l-transparent box-border ${isHealthy ? 'border-emerald-500' : 'border-rose-400'}`}
        ...
        <div className="flex items-center gap-6 mt-4 w-full px-4 text-sm font-medium">
        ...
        <div className={`mt-6 px-4 py-2 rounded-full text-xs font-medium flex items-center gap-1.5 ${isHealthy ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'}`}>
